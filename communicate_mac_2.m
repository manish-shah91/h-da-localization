...
f=2400;                 %MHz
points=11;              %Points in sweep: Choose from: 3, 11, 21, 26, 51, 101, 201, 401, 801, 1601
...
fprintf(obj1,strcat('star',num2str(f),'mhz;'))
fprintf(obj1,strcat('stop',num2str(f),'mhz;'))
...
while(isempty(x))
    ...
    data(:,position_index)=multiple_measurements;
...
if x=='y'
    prompt='file index?';
    x=input(prompt,'s');
    save(strcat(num2str(position_index),'_elements_',num2str(points*points_multiplier),'_sweep_points_',x,'.mat'),'data');
